        ...
        for (k = 0; k < node_port_num; k++) { /* Scan all ports */
            ...
            if ((n > 0) && ((int) in_packet->dst == host_id)) {
                ...
                switch(in_packet->type) {
                    ...
                    case (char) PKT_FILE_UPLOAD_IN:
                        new_job->type = JOB_FILE_UPLOAD_RECV_IN;
                        job_q_add(&job_q, new_job);
                        break;
        ...
        if (job_q_num(&job_q) > 0) {
            ...
            switch(new_job->type) {
                ...
                case JOB_SEND_PKT_ALL_PORTS:
                    for (k=0; k<node_port_num; k++) {
			if(node_port[k].type == SWITCH)
                        	packet_send(node_port[k], 
					    new_job->packet);
                    }
                    free(new_job->packet);
                    free(new_job);
                ...
                case JOB_FILE_UPLOAD_SEND:
                    ...
                    if (dir_valid == 1) {
                        ...
                        if (fp != NULL) {
                            ...
                            printf("\nAdded a start file packet");

                            //////////////////////////////////////
                            /* 1 Create and put packets in queue
                             *   until you've read in less than 100 char
                ...
                case JOB_FILE_UPLOAD_RECV_IN:
                    /* Add packets to the file buff
                     */
